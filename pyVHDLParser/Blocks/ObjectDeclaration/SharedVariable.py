...
class SharedVariableBlock(Block):
	...
	@classmethod
	def stateSharedVariableKeyword(cls, parserState: ParserState):
		...
		if isinstance(token, CharacterToken):
			if (token == "\n"):
				...
				if (not isinstance(parserState.LastBlock, MultiLineCommentBlock)):
					parserState.NewBlock =  SharedVariableBlock(parserState.LastBlock, parserState.TokenMarker, endToken=parserState.NewToken.PreviousToken, multiPart=True)
					_ =                     LinebreakBlock(parserState.NewBlock, parserState.NewToken)
				else:
	...
	@classmethod
	def stateSharedVariableName(cls, parserState: ParserState):
		token = parserState.Token
