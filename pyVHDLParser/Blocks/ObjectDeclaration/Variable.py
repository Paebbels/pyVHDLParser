...
class VariableBlock(Block):
	...
	@classmethod
	def stateVariableKeyword(cls, parserState):
		token = parserState.Token
		errorMessage = "Expected whitespace after keyword VARIABLE."
		if isinstance(token, CharacterToken):
			if (token == "\n"):
				parserState.NewBlock =    VariableBlock(parserState.LastBlock, parserState.TokenMarker, endToken=token.PreviousToken, multiPart=True)
	...
	def stateWhitespace1(cls, parserState):
		token = parserState.Token
		errorMessage = "Expected variable name (identifier)."
		if isinstance(token, CharacterToken):
			if (token == "\n"):
				parserState.NewToken =    LinebreakToken(token)
				if (not isinstance(parserState.LastBlock, MultiLineCommentBlock)):
					parserState.NewBlock =  VariableBlock(parserState.LastBlock, parserState.TokenMarker, endToken=parserState.NewToken.PreviousToken, multiPart=True)
					_ =                     LinebreakBlock(parserState.NewBlock, parserState.NewToken)
				else:
			...
			parserState.NextState =     cls.stateVariableName
			return
	...
	@classmethod
	def stateVariableName(cls, parserState):
		token = parserState.Token
	...
	def stateExpressionEnd(cls, parserState):
		...
		if isinstance(token, CharacterToken):
			if (token == ";"):
				...
				parserState.NewBlock =    VariableBlock(parserState.LastBlock, parserState.TokenMarker, endToken=parserState.NewToken)
				parserState.Pop()
				return
			elif (token == "-"):
				parserState.NewBlock =    VariableBlock(parserState.LastBlock, parserState.TokenMarker, endToken=token.PreviousToken, multiPart=True)
