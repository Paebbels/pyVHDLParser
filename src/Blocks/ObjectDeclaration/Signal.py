...
class SignalBlock(Block):
	def RegisterStates(self):
		return [
			self.stateSignalKeyword,
			self.stateWhitespace1,
			self.stateSignalName,
	...
	@classmethod
	def stateSignalKeyword(cls, parserState):
		token = parserState.Token
		errorMessage = "Expected whitespace after keyword SIGNAL."
		if isinstance(token, CharacterToken):
			if (token == "\n"):
				parserState.NewToken =    LinebreakToken(token)
				parserState.NewBlock =    SignalBlock(parserState.LastBlock, parserState.TokenMarker, endToken=parserState.NewToken, multiPart=True)
	...
	@classmethod
	def stateSignalName(cls, parserState):
		token = parserState.Token
	...
	def stateExpressionEnd(cls, parserState):
		...
		if isinstance(token, CharacterToken):
			if (token == ";"):
				...
				parserState.NewBlock =    SignalBlock(parserState.LastBlock, parserState.TokenMarker, endToken=parserState.NewToken)
				parserState.Pop()
				return
			elif (token == "-"):
				parserState.NewBlock =    SignalBlock(parserState.LastBlock, parserState.TokenMarker, endToken=token.PreviousToken, multiPart=True)
